...
###### 分支管理
```
# 查询分支列表,列表中带*的分支表示当前检出分支，即HEAD指针指向的分支
git branch
# 查看每一个分支的最后一次提交
git branch -v
# 过滤列表中已经合并或尚未合并到当前分支的分支
git branch --merged 或 git branch --no-merged
# 也可指定其他分支的合并状态，例如合并到master的分支
git branch --merged master
```
###### 分支开发工作流
- 长期分支：只在一个分支上保留完全稳定的代码，其他分支稳定后再合入
- 主题分支：短期分支，一些紧急修改分支，或者不同的实践性改动
###### 远程分支
...
```
# 克隆时指定<remote>名称，克隆后本地分支和远程跟踪分支都指向最新的提交对象
git clone -o remotename
```
- 本地与远程的工作可以分叉，只要不拉取数据
```
# 更新你的远程跟踪分支,从中抓取本地没有的数据，并且移动远程跟踪分支指针指向新的位置，但是不会合并，除非使用git pull且当前本地分支跟踪了远程分支
git fetch <remote>
```
- 添加一个新的远程仓库引用到当前的项目
```
...
```
- 抓取新远程仓库有而本地没有的数据，会设置远程跟踪分支newremote/master指向原仓库的master分支（如果是基于原仓库建的仓库，且没有分叉）
- 推送，需要共享一个分支时，需要显示的推送想要分享的分支
```
# 这里的工作被简化了，完整命令应该是git push refs/heads/<branch>:refs/heads/<branch>
...
```
- 推送后，其他协作者从服务器抓取数据时，会在本地生成一个远程分支——指向服务器分支的引用（远程跟踪分支）
- 抓取到新的远程分支时，本地不会自动生成一份可编辑的副本，本地不会有一个新的分支
```
# 将新抓取的内容合并到当前所在的分支
git merge <remote>/<branch>
# 想要在自己的新分支上工作，可以将其建立在远程跟踪分支之上
git checkout -b <branch> <remote>/<branch> 
```
- 跟踪分支，从一个远程跟踪分支检出一个本地分支会自动创建一个叫做“跟踪分支”（上游分支），跟踪分支是远程分支有直接关系的本地分支
```
# 在跟踪分支上使用git pull，Git能自动的识别去哪个服务器抓取、合并到哪个分支
git pull
# 克隆一个仓库时，通常会自动创建一个跟踪origin/master的master分支，也可以设置其他的跟踪分支
git checkout -b <branch> <remote>/<branch>
git checkout --track <remote>/<branch>
# 设置已有的本地分支跟踪一个刚刚拉取下来的远程分支，或者想要修改正在跟踪的上游分支
git branch -u <remote>/<branch> 或 git branch --set-upstream-to <remote>/<branch>
# 上游快捷方式，当设置好跟踪分支后，可通过@{upstream}或@{u}快捷方式来引用它
git merge @{u} 代替 git merge origin/master
# 查看设置的所有跟踪分支,显示每一个分支正在跟踪哪个远程分支，本地分支是否领先、落后或都有，注意这些数据是来自于从服务器最后一次抓取的数据，这个命令并没有连接服务器，只会告诉你关于本地缓存的服务器数据
git branch -vv
或
git fetch --all
git branch -vv
```
- 拉取，```git pull就是git fetch紧接一个git merge```
- 删除远程分支
```
# 假设你已经通过远程分支做完所有工作，或你和协作者已经完成一个特性并且合并到了远程仓库的master分支，可以删除该远程分支
git push <remote> --delete <branch>
# 该命令做的只是从服务器上移除这个指针，Git服务器通常会保留数据一段时间直到垃圾回收运行，若不小新删除，通常是很容易恢复的，如何恢复？？？
```
###### 变基
- 在Git中整合来自不同分支的修改主要有两种方法：merge和rebase
- 变基的基本操作，当提交节点在C1、C2后出现分叉C3和C4时,假设C3是master分支
```
# merge方法：C3,C4,C2来一次三方合并
# rebase方法：提取在C4中引入的修改，在C3的基础上再应用一次（将某一分支上的所有修改都移至另一分支上）
git checkout c4
git rebase c3
# 原理：找到最近公共祖先C2，对比当前分支C4相对于该祖先的历次提交，提取相应修改并存为临时文件，然后将当前分支指向目标基底C3，最后依次将之前另存为临时文件的修改依序应用
# 然后回到c3分支，进行一次快进合并
git checkout c3
git merge c4
# 最终提交对象是一样的，但是变基使得提交历史更加整洁，更加串行化，提交历史是一条直线，没有分叉
```
- 有趣的变基例子：基于master分支，创建了特性分支server，提交修改后再根据server分支，创建了client分支
```
# 假设你希望将client中的修改合并到主分支并发布，但暂时不想合并server的修改
git rebase --onto master server client
# 以上命令：取出client分支，找出处于client分支和server分支的公共祖先之后的修改，然后把它们在master分支上重演一遍
# 然后再快进合并master分支
git checkout master
git merge client
# 最后将server分支的修改也整合进来，以下命令无需切换到server分支再进行变基，更方便
...
- 用变基解决变基，如有上述风险导致的问题：团队中的某人强制推送并覆盖了一些你所基于的提交
```
# 需要检查你做了哪些修改，他们覆盖了哪些修改，Git除了对整个提交计算SHA-1校验和意外，也对本次提交所引入的修改计算校验和，即“patch-id”
git rebase <trackbranch>
# Git会检查：
# 1.哪些提交是我们分支独有的
# 2.其中哪些提交不是合并操作的结果
# 3.哪些提交在对方覆盖更新时并没有被纳入目标分支
# 4.把查到的这些提交应用到跟踪分支上
...
git pull
# 直到把变基命令当作是在推送前清理提交使之整洁的工具，并且只在从未推送至共用仓库的提交上执行变基命令，就不会有事
# 假如你在那些已经被推送至共用仓库的提交执行变基命令，并因此丢弃了一些别人的开发所基于的提交，那你就有大麻烦了，你的同事也会因此鄙视你
```
- 变基 vs 合并：1种观点认为合并属于记录实际发生过什么，这些痕迹应该保留下来，另一种观点认为提交历史是项目过程中发生的故事，应该怎么方便后来的读者就怎么写
